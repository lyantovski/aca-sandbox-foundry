...
async def handle_task(request: Request, _auth: None = Depends(verify_a2a_token)):
    ...
    try:
        result = await run_research(topic, preferences, task_id=task_id)
...
@app.get("/status")
async def status(task_id: str | None = None):
    """Live status endpoint — polled by Dev UI to show sandbox progress before brief is ready."""
    if task_id and _agent_module._live_status.get("task_id") != task_id:
        return JSONResponse(
            {
                "task_id": task_id,
                "phase": "pending",
    ...
    return JSONResponse(_agent_module._live_status)
