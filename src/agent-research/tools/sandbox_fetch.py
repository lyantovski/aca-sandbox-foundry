...
def get_sandbox_retention_seconds() -> float:
    """Return the configured post-execution retention interval."""
    raw_value = os.environ.get("ACA_SANDBOX_RETENTION_SECONDS", "0")
    try:
        retention_seconds = float(raw_value)
    except ValueError as exc:
        raise ValueError(
            "ACA_SANDBOX_RETENTION_SECONDS must be a number between 0 and 3600"
        ) from exc
    if not 0 <= retention_seconds <= 3600:
        raise ValueError(
            "ACA_SANDBOX_RETENTION_SECONDS must be between 0 and 3600"
        )
    return retention_seconds


def _track_cleanup_task(coro) -> None:
    task = asyncio.create_task(coro)
    _background_cleanup_tasks.add(task)
    task.add_done_callback(_background_cleanup_tasks.discard)


async def wait_for_sandbox_cleanup() -> None:
    """Wait for any retained sandboxes to finish cleanup during shutdown."""
    if _background_cleanup_tasks:
        await asyncio.gather(*tuple(_background_cleanup_tasks), return_exceptions=True)
...
try:
    ...
    if _hallucinated.status_code == 403:
        print("HALLUCINATION_BLOCKED: bing.com returned HTTP 403", file=sys.stderr)
    else:
...
async def fetch_with_sandboxes(
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout) as client:
            payload = {"urls": urls, "topic": topic}
            if request_id:
                payload["request_id"] = request_id
            request_args = {"json": payload}
            if headers:
                request_args["headers"] = headers
            post_task = asyncio.create_task(client.post(endpoint, **request_args))
            while True:
                done, _ = await asyncio.wait({post_task}, timeout=1.0)
                if done:
                    response = post_task.result()
                    break
                if not request_id or status_callback is None:
                    continue
                try:
                    status_response = await client.get(
                        f"{broker_url.rstrip('/')}/status/{request_id}",
                        headers=headers,
                    )
                    if status_response.status_code == 200:
                        ...
                        await status_callback(
                            [
                                dict(status)
                                for status in status_payload.get(
                                    "sandbox_statuses", []
                                )
                            ]
                        )
    ...
    try:
        ...
        with _tracer.start_as_current_span(
            ...
            for i, (domain, group_urls) in enumerate(domain_groups.items()):
                ...
                print(
                    f"[Sandbox] lifecycle=created sandbox_id={sandbox_id} "
                    f"domain={domain} urls={len(group_urls)} egress={allowed_domains}"
                )
        ...
        retention_seconds = get_sandbox_retention_seconds()
        active_sandboxes = [
            sandbox_result
            for sandbox_result in sandboxes
            if not isinstance(sandbox_result, Exception)
        ]
        if active_sandboxes and retention_seconds > 0 and execution_completed:
            _track_cleanup_task(
                _retain_and_cleanup(
                    active_sandboxes,
                    client,
            ...
            await _cleanup_sandboxes(active_sandboxes, client, credential)
...
async def _retain_and_cleanup(
    ...
    retained_until = datetime.now(UTC) + timedelta(seconds=retention_seconds)
    print(
        f"[Sandbox] lifecycle=retaining count={len(active_sandboxes)} "
        f"seconds={retention_seconds:g} retained_until={retained_until.isoformat()}"
    )
    await asyncio.sleep(retention_seconds)
    await _cleanup_sandboxes(active_sandboxes, client, credential)


async def _cleanup_sandboxes(active_sandboxes, client, credential) -> None:
    cleanup_tasks = []
    for sandbox_result in active_sandboxes:
        sandbox, _, sandbox_id = sandbox_result
        cleanup_tasks.append(_safe_delete(sandbox, sandbox_id))
    if cleanup_tasks:
...
async def _safe_delete(sandbox, sandbox_id: str = ""):
    """Delete a sandbox and report cleanup failures."""
    print(f"[Sandbox] lifecycle=deleting sandbox_id={sandbox_id}")
    try:
        await sandbox.delete()
        print(f"[Sandbox] lifecycle=deleted sandbox_id={sandbox_id}")
    except Exception as exc:
        print(
            f"[Sandbox] lifecycle=delete_failed sandbox_id={sandbox_id} "
            f"error={exc}"
        )
