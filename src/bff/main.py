...
def _oauth_identity(request: Request) -> dict[str, str]:
    user = (
...
def create_app() -> FastAPI:
    ...
    async def require_oauth_identity(request: Request, call_next):
        if request.url.path != "/health" and app.state.auth_required:
            identity = _oauth_identity(request)
            if not identity["user"] and not identity["email"]:
                return JSONResponse(
                    status_code=401,
    ...
    async def current_user(request: Request) -> dict[str, str]:
        return _oauth_identity(request)
